...
7. **PDF logo branding** is a known upcoming item. If a logo is added to the report it becomes a new offline asset dependency; decide now whether it is embedded in the bundle or cached as a data URI, not after this work lands.
   **Decided (Sep 2026):** the testing company's logo is cached as a **data URI in the per-tester IndexedDB reference store** (`companyLogo:{companyId}`), refreshed on app load and after every sync by a conditional GET against a content-hash ETag — not via the SW runtime cache, so a report prints the logo offline even if it was never displayed online. See `Client/sync/companyLogoSync.ts`.