...
**RECOMMENDATION: (1) mechanically, but stored in the per-tester IndexedDB rather than localStorage, and paired with an explicit product rule: _offline, a device belongs to one tester._** Concretely: the shell opens the last-known tester's DB; the header shows whose session it is; switching testers requires connectivity. This is honest about what is actually enforceable — no client-side scheme survives a determined holder of an unlocked device, and device passcode/MDM is the real control. Do **not** ship the localStorage variant without that rule stated, and do **not** ship it without the Phase 0 purge guard.
