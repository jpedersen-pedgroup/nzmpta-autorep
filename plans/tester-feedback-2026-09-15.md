...
- 🔎 The rate/ratio **spread** check is computed over the rows entered (`Client/passfail/pulsatorStats.ts:53-56`). With failed-only entry a single slow unit shows "spread 0 ppm ≤ 6 PASS". Legacy captured the machine-level figures explicitly — "Rate Range Fastest / Slowest", "Ratio Range Highest / Lowest" (⏱ 23:58). Capture those four as readings here and drop the computed spread badge (screen and report). Pre-existing; noted during the Phase 0 review.
